"""Build a focused preview navigator for converted pages 51-100."""
...
audit = json.loads((ROOT / "content/review-pages-051-100.json").read_text(encoding="utf-8"))
pages = audit["pages"]
buttons = "".join(
...
<title>Preview Pages 51-100</title><style>
...
</style></head><body><header>Sayansi ADT - Preview ya converted pages 51-100</header><main><nav>{buttons}</nav><iframe title="Preview ya ukurasa"></iframe></main>
<script>const frame=document.querySelector('iframe'),buttons=[...document.querySelectorAll('button')];function openPage(b){{buttons.forEach(x=>x.classList.remove('active'));b.classList.add('active');frame.src=b.dataset.href+'?preview100=1';history.replaceState(null,'','#page-'+b.dataset.page)}}buttons.forEach(b=>b.onclick=()=>openPage(b));const requested=location.hash.replace('#page-','');openPage(buttons.find(b=>b.dataset.page===requested)||buttons[0]);</script>
</body></html>'''
(ROOT / "preview-pages-051-100.html").write_text(document, encoding="utf-8")
